package com.app.lib_widget.text;
...
//                    }
    ...
    private void switchOpenClose() {
        //判断是否额外截断，如果有截断接口，就实现截断信息返回
        if(interceptSwitchOpenClose!=null){
            interceptSwitchOpenClose.callBack();
     ...
     * 拦截点击事件的处理
     * */
   public interface InterceptSwitchOpenClose{
        void callBack();
    }

    private InterceptSwitchOpenClose interceptSwitchOpenClose;

    public void setInterceptSwitchOpenClose(InterceptSwitchOpenClose interceptSwitchOpenClose){
        this.interceptSwitchOpenClose=interceptSwitchOpenClose;
    }
