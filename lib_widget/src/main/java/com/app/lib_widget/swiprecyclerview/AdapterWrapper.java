 ...
 */
package com.app.lib_widget.swiprecyclerview;
...
import static com.app.lib_widget.swiprecyclerview.SwipeRecyclerView.LEFT_DIRECTION;
import static com.app.lib_widget.swiprecyclerview.SwipeRecyclerView.RIGHT_DIRECTION;
...
class AdapterWrapper extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private OnCallBackListener mOperatorCallBack;//操作角标回调

    AdapterWrapper(Context context, RecyclerView.Adapter adapter) {
        this.mInflater = LayoutInflater.from(context);
        this.mAdapter = adapter;
        this.mOperatorCallBack=new OnCallBackListener() {
            @Override
            public void mainCallBack(Object entity) {
               operatorPosition= (int) entity;
            }
        };
    }
    ...
    //菜单栏回收就重置当前点击状态
    boolean confirmOperator=false;//二次确认标识,默认不用二次确认

    String confirmText = "";//二次确认文案

    int confirmIndex ;//二次确认角标

    public void setConfirmInfo(boolean confirmOperator, String confirmText, int confirmIndex) {
    ...
    public void resetConfirmText(){
        if(confirmOperator){
            if(operatorPosition>-1){
                ...
                notifyItemChanged(operatorPosition);
    ...
    public final void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position,
        ...
        if (itemView instanceof SwipeMenuLayout && mSwipeMenuCreator != null) {
            ...
            if (leftMenu.hasMenuItems()) {
                ...
                leftMenuView.createMenu(holder, leftMenu, menuLayout, LEFT_DIRECTION, mOnItemMenuClickListener,confirmOperator,confirmText,confirmIndex,mOperatorCallBack);
            ...
            if (rightMenu.hasMenuItems()) {
                ...
                rightMenuView.createMenu(holder, rightMenu, menuLayout, RIGHT_DIRECTION, mOnItemMenuClickListener,confirmOperator,confirmText,confirmIndex,mOperatorCallBack);
